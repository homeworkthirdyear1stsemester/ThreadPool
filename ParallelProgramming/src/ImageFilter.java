...
public class ImageFilter {
    ...
    public ImageFilter(String path) {
        this.image = null;
        try {
            File file = new File(Paths.get(path).toString());//파일 객체 생성
            BufferedImage bufferedImage = ImageIO.read(file);//읽어옴
            Raster raster = bufferedImage.getRaster();
            width = raster.getWidth();//크기 지정
            height = raster.getHeight();//그키지정

            image = new int[height][width];//배열 초기화
            filter_image = new int[height][width];//filter된 이미지 초기화
    ...
    public void applyMedianFilter() {//시간 측정
